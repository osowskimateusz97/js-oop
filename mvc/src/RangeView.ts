...
export class RangeView extends View{
    ...
    updateFields(payload: InputPayload) {
        if(Array.isArray(payload)) return console.error('It should be object!');
        const {currencyType, euValue, currencyValue} = payload;
        const euElement = <HTMLInputElement>this.getElement(`#${currencyType} input[data-type="eu"]`);
